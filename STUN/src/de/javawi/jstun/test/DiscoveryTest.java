...
public class DiscoveryTest
{
	...
	public DiscoveryInfo test() throws UtilityException,
			SocketException,
	...
	private boolean test1() throws UtilityException,
			SocketException,
			UnknownHostException,
			IOException,
			MessageAttributeParsingException,
			MessageHeaderParsingException
	{
		...
		while (true)
		{
			try
			{
				...
				socketTest1 =
						new DatagramSocket(new InetSocketAddress(iaddress, 0));
				...
				while (!(receiveMH.equalTransactionID(sendMH)))
				{
					DatagramPacket receive =
							new DatagramPacket(new byte[200], 200);
				...
				ma =
						(MappedAddress) receiveMH
								.getMessageAttribute(MessageAttribute.MessageAttributeType.MappedAddress);
				ca =
						(ChangedAddress) receiveMH
				...
				} else
				{
					...
					if ((ma.getPort() == socketTest1.getLocalPort()) && (ma
							.getAddress().getInetAddress().equals(socketTest1
							.getLocalAddress())))
					{
	...
	private boolean test2() throws UtilityException,
			SocketException,
			UnknownHostException,
			IOException,
			MessageAttributeParsingException,
		...
		while (true)
		{
			try
			{
				...
				DatagramSocket receiveSocket =
						new DatagramSocket(localPort, localAddress);
	...
	private boolean test1Redo() throws UtilityException,
			SocketException,
			UnknownHostException,
			IOException,
			MessageAttributeParsingException,
			...
			try
			{
				...
				} else
				{
					if ((ma.getPort() != ma2.getPort()) || (!(ma.getAddress()
							.getInetAddress().equals(ma2.getAddress()
							.getInetAddress()))))
					{
						di.setSymmetric();
	...
	private void test3() throws UtilityException,
			SocketException,
			UnknownHostException,
			IOException,
			MessageAttributeParsingException,
