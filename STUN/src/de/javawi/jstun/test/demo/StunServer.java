...
public class StunServer
{
	...
	public void start() throws SocketException
	{
		for (DatagramSocket socket : sockets)
		{
			...
			StunServerReceiverThread ssrt =
					new StunServerReceiverThread(socket);
			ssrt.start();
		}
	}
	...
	class StunServerReceiverThread extends Thread
	{
		...
		StunServerReceiverThread(DatagramSocket datagramSocket)
		{
			...
			for (DatagramSocket socket : sockets)
			{
				if ((socket.getLocalPort() != receiverSocket.getLocalPort()) && (socket
						.getLocalAddress().equals(receiverSocket
						.getLocalAddress())))
					changedPort = socket;
				if ((socket.getLocalPort() == receiverSocket.getLocalPort()) && (!socket
						.getLocalAddress()
						.equals(receiverSocket.getLocalAddress())))
					changedIP = socket;
				if ((socket.getLocalPort() != receiverSocket.getLocalPort()) && (!socket
						.getLocalAddress()
						.equals(receiverSocket.getLocalAddress())))
				...
				try
				{
					...
					try
					{
						...
						if (receiveMH.getType() == MessageHeaderType.BindingRequest)
						{
							...
							ResponseAddress ra =
									(ResponseAddress) receiveMH
											.getMessageAttribute(MessageAttributeType.ResponseAddress);

							MessageHeader sendMH =
									new MessageHeader(MessageHeaderType.BindingResponse);
							...
							if (cr.isChangePort() && (!cr.isChangeIP()))
							{
								...
								LOGGER.debug(changedPort.getLocalAddress()
										.getHostAddress() + ":"
							...
							} else if ((!cr.isChangePort()) && cr.isChangeIP())
							{
								...
								changedIP.send(send);
								LOGGER.debug(changedIP.getLocalAddress()
										.getHostAddress() + ":"
					...
					} catch (UnknownMessageAttributeException umae)
					{
						...
						MessageHeader sendMH =
								new MessageHeader(MessageHeaderType.BindingErrorResponse);
	...
	public static void main(String args[])
	{
		try
		{
			...
			java.util.logging.Logger.getLogger("de.javawi.jstun")
					.setLevel(Level.ALL);
			StunServer ss =
					new StunServer(Integer.parseInt(args[0]),
