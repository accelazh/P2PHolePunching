...
public class DiscoveryTestDemo implements Runnable
{
	...
	public void run()
	{
		...
		{
			DiscoveryTest test =
					new DiscoveryTest(iaddress, "jstun.javawi.de", 3478);
			// DiscoveryTest test =
			// new DiscoveryTest(iaddress, "stun.sipgate.net", 10000);
		...
		try
		{
			...
			while (ifaces.hasMoreElements())
			{
				...
				while (iaddresses.hasMoreElements())
				{
					...
					if (Class.forName("java.net.Inet4Address")
							.isInstance(iaddress))
					{
						...
						{
							Thread thread =
									new Thread(new DiscoveryTestDemo(iaddress));
							thread.start();
						}
					}
				}
